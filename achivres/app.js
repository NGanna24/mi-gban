...
cron.schedule('*/5 * * * *', async () => {
    ...
    try {
        const result = await VisitReminderService.checkAndSendReminders();
...
// });
app.use((req, res, next) => {
    // Seulement rediriger HTTPS en production et pour le domaine réel
    if (process.env.NODE_ENV === 'production' && 
        (req.headers.host === 'n-double.com' || req.headers.host === 'www.n-double.com') &&
        req.headers['x-forwarded-proto'] !== 'https' &&
        !req.originalUrl.includes('/health')) {
        return res.redirect(`https://${req.headers.host}${req.url}`);
    }
...
// Service des documents d'agents
app.use('/uploads/agent-documents', express.static(agentDocumentsDir, {
    setHeaders: (res, filePath) => {
        res.setHeader('Access-Control-Allow-Origin', '*');
        res.setHeader('Cache-Control', 'public, max-age=86400');
        
        // Définition des types MIME pour les documents
        if (filePath.endsWith('.jpg') || filePath.endsWith('.jpeg')) {
            res.setHeader('Content-Type', 'image/jpeg');
        } else if (filePath.endsWith('.png')) {
            res.setHeader('Content-Type', 'image/png');
